# method3: concat all lines into a huge string
# this method is not agile for section names. For example normally it's ###  **Details, if it's ## **Details, there would be error
import os
os.chdir(r"D:\full_stack\py\Markdown\test")

#%% read original file ----------------------------------
nameScript='Attachement size check.md'
...
try:
    with open(nameScript[:len(nameScript)-3]+'_new.md', "w", encoding="utf-8") as output_file:
        ...
        print('congrats, output succeeded')
except:
	print('Something evil\'s lurking in the dark')